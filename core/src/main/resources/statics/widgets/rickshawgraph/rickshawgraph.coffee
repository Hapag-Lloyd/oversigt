...
class Dashing.Rickshawgraph extends Dashing.Widget
    ...
    if answer == null
      ...
      if !(series?.length > 0)
        ...
        switch @get('summaryMethod')
          ...
          when "highestLatest"
            answer = 0
            answer = Math.max(answer, s.data[s.data.length - 1].y or 0) for s in series
  ...
  ready: ->
    @assignedColors = @get('colors').split(':').reverse() if @get('colors')
  ...
  _createGraph: ->
    ...
    width  = $node.width()#(Dashing.widget_base_dimensions[0] * $container.data("sizex")) + Dashing.widget_margins[0] * 2 * (($container.data("sizex") ? 1) - 1) - 15
    height = $node.height()#(Dashing.widget_base_dimensions[1] * $container.data("sizey")) + Dashing.widget_margins[1] * 2 * (($container.data("sizey") ? 1) - 1)
