...
window.TimeKnots = {
    draw: function(id, events, options) {
        ...
        TIMELINE_LEFT_MARGIN = 160;
        TEXT_LEFT_MARGIN = 140;
        DATE_LEFT_MARGIN = 130;
        ...
        svg.selectAll("circle")
            ...
            .attr("cy", function(d) {
                ...
                var datum = (cfg.dateDimension) ? new Date(d.date).getTime() : d.value;
                var cy = Math.floor(step * (datum - minValue) + margin);
                return TimeKnots.checkAndFixCollision(svg, ".timeline-event", cy, this);
            })
            .attr("cx", function(d) {
                if (cfg.horizontalLayout) {
                    var datum = (cfg.dateDimension) ? new Date(d.date).getTime() : d.value;
    ...
    checkAndFixCollision: function(svg, id, y, that) {
        svg.selectAll(id).each(function() {
            if (this != that) {
                var thisY = parseInt(this.getAttribute('y')) || parseInt(this.getAttribute('cy'));
                if (thisY && y < thisY + 16) {
                	y = thisY + 16;
                }
            }
