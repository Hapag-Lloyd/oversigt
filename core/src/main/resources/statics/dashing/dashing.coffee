...
class Dashing.Widget extends Batman.View
  ...
  @::on 'ready', ->
    ...
    if lastData
      @receiveData(lastData)

  receiveData: (data) =>
    if data.error
      if $(@node).find('.eventSourceError').length < 1
        $(@node).append('<div class="eventSourceError"><h1>Error</h1><p></p></div>')
      $(@node).parent().addClass('error')
      $(@node).find('.eventSourceError p').text(data.errorMessage);
    else
      $(@node).parent().removeClass('error')
      clone = $(@node).clone().insertAfter(@node).css('opacity', 0.999999)
      @mixin(data)
      @onData(data, (setTimeout (-> clone.remove()), 1000))
...
# NEUMAOL hacking: send the dashboard name so events can be filtered on server
source = new EventSource('events?dashboard=' + window.location.pathname.substr(1))
