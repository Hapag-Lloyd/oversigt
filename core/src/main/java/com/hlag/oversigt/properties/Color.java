...
public class Color {
	...
	private static Color parseHexColor(String string) {
		if (string.length() == 3) {
			String r = string.substring(0, 1);
			String g = string.substring(1, 2);
	...
	private static Color parseFunctionColor(String string) {
		...
		if (matcher.find()) {
			...
			switch (function.toLowerCase()) {
				...
				case "hsl":
				case "hsla":
					int[] rgb = hslToRgb(Integer.parseInt(r), Integer.parseInt(r), Integer.parseInt(b));
	...
	public static Color smooth(Color start, Color end, double progress) {
		int r = smooth(start.getRed(), end.getRed(), progress);
		int g = smooth(start.getGreen(), end.getGreen(), progress);
		int b = smooth(start.getBlue(), end.getBlue(), progress);
		return new Color(r, g, b);
	}

	private static int smooth(int start, int end, double progress) {
