...
public class OversigtServer extends AbstractIdleService {
	...
	private HttpHandler withLogin(HttpHandler handler) {
		return withSession(Handlers.predicate(HttpUtils::hasSession, handler, loginHandler));
	}
	...
	private void doLogout(HttpServerExchange exchange) {
		Optional<Principal> principal = HttpUtils.getPrincipal(exchange);
		if (principal.isPresent()) {
			HttpUtils.getSession(exchange).ifPresent(s -> s.invalidate(exchange));
